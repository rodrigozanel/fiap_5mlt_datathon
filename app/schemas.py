...
class StudentInput(BaseModel):
    ...
    fase: int = Field(
        ge=0, le=8, description="Fase do aluno na Passos Magicos (0=ALFA)"
    )
    ...
    inde: float = Field(
        ge=0, le=12, description="Indice de Desenvolvimento Educacional"
    )
    pedra: str = Field(
        description="Classificacao PEDRA: Quartzo, Ágata, Ametista ou Topázio"
    )
    ...
    ipp: float = Field(
        default=0.0, ge=0, le=12, description="Indicador Psicopedagogico"
    )
